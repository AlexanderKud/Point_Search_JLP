...
auto main() -> int {
    ...
    Point offset_point = secp256k1->SubtractPoints(puzzle_point, P_table[range_start]);

    ofstream outFile; // writing stride_sum to file (initial value = 0)
    outFile.open("stride_sum.txt");
    outFile << "0" << '\n';
    outFile.close();

    ofstream outFileP; // writing offset_point to file
    outFileP.open("offset_point.txt");
    outFileP << secp256k1->GetPublicKeyHex(offset_point) << '\n';
    outFileP.close();
    ...
    auto bloom_create = [&]() {
        ...
        Point P = secp256k1->SubtractPoints(offset_point, secp256k1->G);
