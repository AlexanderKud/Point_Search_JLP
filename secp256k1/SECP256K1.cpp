...
void Secp256K1::Init() {
  ...
  Infinity.x.SetInt32(0);
  Infinity.y.SetInt32(0);
  Infinity.z.SetInt32(1);
  ...
  // Compute table for FastScalarMultiplication
  Int pk; pk.SetBase10("2");
  uint64_t mult = 2;
  for (int i = 0; i < 256; i++) {
      addend_points[i] = ScalarMultiplication(&pk);
      pk.Mult(mult);
  }
...
Point Secp256K1::ScalarMultiplication(Int *scalar) {
  Point R;
  int  no_of_bits, loop;
  ...
  for(loop = no_of_bits - 2; loop >= 0; loop--) {
    R = DoubleDirect(R);
    if(scalar->GetBit(loop)) { R = AddDirect(R, G); }        
  }
  return R;
}

Point Secp256K1::ScalarMultiplication2(Int *scalar) {
  Point R;
  int  no_of_bits, loop;
  no_of_bits = scalar->GetBitLength();
  R.Set(G);
  ...
  for(loop = no_of_bits - 2; loop >= 0; loop--) {
    R = Double(R);
    if(scalar->GetBit(loop)) { R = Add2(R, G); }      
  }
  R.Reduce();
  return R;
}

Point Secp256K1::AddDirect2(Point &p1, Point &p2) {

  if (p1.equals2(Infinity)) { return p2; }
...
Point Secp256K1::FastScalarMultiplication(Int *scalar) {
  Point Result; Result.Set(Infinity);
  Point Addend; Addend.Set(G); 
  int no_of_bits, loop;
  no_of_bits = scalar->GetBitLength();
  for(loop = 0; loop < no_of_bits ; loop++) {
    if(scalar->GetBit(loop)) { Result = AddDirect2(Result, Addend); }
    Addend = addend_points[loop];
  }
  return Result;
}

Point Secp256K1::FastScalarMultiplication2(Int *scalar) {
  Point Result; Result.Set(Infinity);
  Point Addend; Addend.Set(G); Addend.z.SetInt32(1);
  int no_of_bits, loop;
  no_of_bits = scalar->GetBitLength();
  for(loop = 0; loop < no_of_bits ; loop++) {
    if(scalar->GetBit(loop)) { Result = Add2(Result, Addend); }
    Addend = addend_points[loop];
  }
  Result.Reduce();
  return Result;
...
Point Secp256K1::PointMultiplication(Point &P, Int *scalar) {
  ...
  for(loop = no_of_bits - 2; loop >= 0; loop--) {
      R = DoubleDirect(R);
      if(scalar->GetBit(loop)) { R = AddDirect(R, T); }        
  }
  return R;
}

Point Secp256K1::PointMultiplication2(Point &P, Int *scalar) {
  Point R, T;
  int  no_of_bits, loop;
  no_of_bits = scalar->GetBitLength();
...
Point Secp256K1::PointDivision2(Point &P, Int *scalar) {
  Point A;
  Int mod_inv;
  mpz_t N, d, s;
  mpz_inits(N, d, s, NULL);
  ...
  mod_inv.SetBase10(mpz_get_str(NULL, 10, d));
  mpz_clears(N, d, s, NULL);
  A = PointMultiplication2(P, &mod_inv);
  return A;
}
...
Point Secp256K1::ParsePublicKeyHex2(std::string str) {
...
Point Secp256K1::Add2(Point &p1, Point &p2) {
  if (p1.equals2(Infinity)) { return p2; }
  if (p2.equals2(Infinity)) { return p1; }
  // P2.z = 1
  if (p1.equals(p2)) {
      Point r;
      r = Double(p1);
...
Point Secp256K1::Subtract(Point &p1, Point &p2) {
  if (p1.equals2(Infinity)) { return p2; }
  if (p2.equals2(Infinity)) { return p1; }
  if (p1.equals2(p2)) { return Infinity; }
  Point Q1, Q2;
  Q1.Clear(); 
  Q2.Clear();
  Q1 = Negation(p2);
